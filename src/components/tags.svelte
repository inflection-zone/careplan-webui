<script>
    ...
    <div class=" flex  items-center textarea textarea-info textarea-bordered flex-wrap w-full" class:sti-layout-disable={disable} bind:this={layoutElement}>
        <label for={id} class={labelShow ? "" : "sr-only"}>{labelText}</label>
    
        {#if tags.length > 0}
            {#each tags as tag, i}
                <span class="svelte-tags-input-tag flex  bg-[#7165E3] text-white m-2">
        ...
        <input
            ...
            on:click={onClick}
            class="svelte-tags-input flex m-o mt-1 outline-none"
            placeholder={placeholder}
            disabled={disable}
        >
    </div>
